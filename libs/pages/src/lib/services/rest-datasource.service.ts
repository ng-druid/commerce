...
@Injectable()
export class RestDatasourceService extends DataSource<any> {
  private dataStream = new BehaviorSubject<Array<any>>([]);
  ...
  constructor(/*es: EntityServices*/private contentPlugin: ContentPlugin) {
    super();
    this.contentPlugin.handler.buildDynamicItems(c.settings, `panel_${this.counter}_pane_${index}_data`).pipe(
